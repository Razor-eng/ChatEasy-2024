...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        const conversationsWithDetails = await Promise.all(
            conversations.map(async (conversation, id) => {
                const conversationMemberships = await ctx.db
                    .query("conversationMembers")
                    .withIndex(
                        "by_conversationId",
                ...
                const lastSeenMessage = conversationMemberships[id].lastSeenMessage ? await ctx.db.get(conversationMemberships[id].lastSeenMessage!) : null;
